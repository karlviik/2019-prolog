...
:- dynamic ruut/3,ruut_old/3,võttis/1,siire/3,siire_1/3,moves_abi/2,valekäik/1,move_count/1, modeCntr/1.

%mustad(main).	%%%%%%%%%%%%%%%% Käsitsi tehtud ainult testimise ajaks!!!!
%valged(main).	%%%%%%%%%%%%%%%% Käsitsi tehtud ainult testimise ajaks!!!!
...
make_move(Color,ColorNN):-
	...
	millega_käia(ColorN,(X,Y),(X1,Y1),ColorNN)),	% Kui võttis, vaatab kas sama nupuga saab veel võtta
...
%----------------------
millega_käia(Color,(X,Y),(0,0),ColorNN):-		% Kui ei saa edasi võtta, siis saab käiguõiguse vastane
	ruut_old(X,Y,Z),
...
millega_käia(Color,(X,Y),(X,Y),ColorNN):-
...
verify_turn(Color):-						% Võrdle ruut_old/3 ja ruut/3 fakte ja
	käiv_nupp(Color,(X,Y),Kuhu,Võttis),		% Leia mis nupp käis
...
verify_turn(Color):-
	...
	assert(valekäik(Color)),
	retractall(moves_abi(_,_)),!.

käiv_nupp(Color,(X0,Y0),(X2,Y2),Võttis):-
...
nupuga(X0, Y0,Color,XSuund,Käigud):-
	vota_nupuga(X0,Y0,Color,(1,1), Käik1),					% Genereeri nupuga võimalikud võtmised suunas (1,1)
	vota_nupuga(X0,Y0,Color,(1,-1), Käik2),	append(Käik1,Käik2,Käik12),	% Genereeri nupuga võimalikud võtmised suunas (1,-1)
	vota_nupuga(X0,Y0,Color,(-1,1), Käik3),	append(Käik12,Käik3,Käik123),	% Genereeri nupuga võimalikud võtmised suunas (-1,1)
	vota_nupuga(X0,Y0,Color,(-1,-1), Käik4),append(Käik123,Käik4,Käik1234),	% Genereeri nupuga võimalikud võtmised suunas (-1,-1)
		(Käik1234=[],						% Kui vähemalt ühe on nupuga võetud, siis lihtsaid käike enam ei vaadata
	käi_nupuga(X0,Y0,Color,(XSuund,1),Käik5),
	käi_nupuga(X0,Y0,Color,(XSuund,-1),Käik6),
	append(Käik5,Käik6,Käigud);
	Käigud=Käik1234),!.
%------------------ nupuga võtmised-------------
...
käi_nupuga(X0,Y0,_,(DX,DY),[[[],[(X1,Y1)]]]):-
	X1 is X0+DX, Y1 is Y0 + DY,      kas_laual(X1,Y1),
	ruut_old(X1,Y1,0),!.
käi_nupuga(_,_,_,_,[]).

%====================== Tammi käigud antud positsioonilt ==============================================
tammiga(X0, Y0,Käigud):-
	...
	käi_tammiga([(X0,Y0)], VabadRd, Vabad0), delete(Vabad0, (X0,Y0), Vabad),
	append([[]],[Vabad],Käigud0),Käigud=[Käigud0];							% kui ei, tagastab mis ruudud on nendest liikumiseks vabad
	Käigud=Post_posits).
%------------------ Tammiga võtmised-------------
